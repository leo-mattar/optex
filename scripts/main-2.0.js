  ...
  function moveToMobileLayout() {
    ...
    controlsItems.forEach((controlsItem, index) => {
      ...
      if (correspondingTabberItem) {
        correspondingTabberItem.parentElement.insertBefore(controlsItem,
...
// --- PDT TABBER
function pdtTabber() {
  const invisibleTabs = document.querySelectorAll('.c-pdt-tabber-link.w-condition-invisible');
  invisibleTabs.forEach(tab => {
    tab.remove();
  });

  const invisibleTabContent = document.querySelectorAll('[pdt-tabber-name].w-condition-invisible');
  ...
  const firstTabContent = document.querySelector(`[pdt-tabber-name="${firstTabName}"]`);
  if (firstTabContent) {
  ...
  tabLinks.forEach((link) => {
    link.addEventListener('click', function (e) {
      ...
      const targetElement = document.querySelector(`[pdt-tabber-name="${tabName}"]`);
      if (targetElement) {
...
function filterSettings() {
  ...
  function updateTitle() {
    const checkedRadio = filterGroup.querySelector('input[type="radio"][data-tag]:checked');

    if (checkedRadio) {
      titleElement.textContent = checkedRadio.getAttribute('data-tag');
    } else {
      const defaultText = filterResults ? filterResults.getAttribute('data-default-title') : null;
...
function paginationScrollTo() {
  ...
  function attachEventDelegation() {
    ...
    document.addEventListener('click', (e) => {
      const paginationBtn = e.target.closest('.c-pagination-page-btn, .c-pagination-btn');

      if (paginationBtn) {
        const filterGrids = document.querySelectorAll('.o-grid.filter-pdt, .c-pagination');
...
function headerTabber() {
  ...
  const sideLinks = header.querySelectorAll(".c-dd-side-link");
  const panels = header.querySelectorAll(".c-dd-p-list-inner");
...
function insightsPrevNext() {
  ...
  window.FinsweetAttributes.push([
    'list',
    (listInstances) => {
      const title = document.querySelector("[fs-list-element='next-item'] .t-display-4");
...
function faqPage() {
  ...
  function updateTitle() {
    ...
    if (checkedRadio) {
      const label = checkedRadio.closest(".c-faq-filter-radio-btn-field").querySelector(
  ...
  const radioInputs = document.querySelectorAll(".c-faq-filter-form-block input[type='radio']");
  ...
  document.addEventListener('click', function (event) {
    ...
    if (sideFilterItem) {
      ...
      const radioLabels = document.querySelectorAll('.c-faq-filter-radio-label');
...
function careersJobBoard() {
  ...
  const allBtn = document.querySelector(".c-jb-filter-radio-btn-field[data-jb='all-btn']");
...
function resourcesHub() {
  ...
  function updateButtonText(button) {
    const btnText = button.querySelector('.c-btn-txt');
    const parentAccordion = button.closest('.c-rh-ac');

    if (!btnText || !parentAccordion) return;

    const isOpen = parentAccordion.classList.contains('is-open');
    const newText = isOpen ? 'View less' : 'View more';
  ...
  buttons.forEach(button => {
    button.addEventListener('click', () => {
      updateButtonText(button);
      setTimeout(() => updateButtonText(button), 50);
      setTimeout(() => updateButtonText(button), 150);
      setTimeout(() => updateButtonText(button), 300);
    });
  });

  const accordions = document.querySelectorAll('.c-rh-ac');

  accordions.forEach(accordion => {
    const observer = new MutationObserver(mutations => {
      mutations.forEach(mutation => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
          const button = accordion.querySelector('.c-rh-ac-toggle .c-btn');
          if (button) {
            updateButtonText(button);
          }
        }
  ...
  setInterval(() => {
    buttons.forEach(button => {
      ...
      if (btnText && parentAccordion) {
        const expectedText = parentAccordion.classList.contains('is-open') ? 'View less' :
          'View more';
        if (btnText.textContent !== expectedText) {
          updateButtonText(button);
        }
      }
    });
...
function requestLayourForm() {
  ...
  function validateRequiredFields(container, showVisualErrors = false) {
    const requiredFields = container.querySelectorAll('.c-form-input[required]');
    const invalidFields = [];
  ...
  function switchToStep(fromStep, toStep) {
    [`[data-tab='${fromStep}']`, `[data-tab-cta='${fromStep}']`, `[data-tab-title='${fromStep}']`]
    .forEach(selector => {
      const element = form.querySelector(selector);
      if (element) element.classList.remove('is-active');
    });

    [`[data-tab='${toStep}']`, `[data-tab-cta='${toStep}']`, `[data-tab-title='${toStep}']`].forEach
      (selector => {
        const element = form.querySelector(selector);
  ...
  if (step1Button) {
    step1Button.addEventListener('click', function (e) {
      ...
      if (invalidFields.length > 0) {
        ...
        const message =
          `Please correct the highlighted fields before proceeding. ${fieldCount} field${fieldCount > 1 ? 's need' : ' needs'} attention.`;
    ...
    const observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          if (node.nodeType === 1) {
            if (node.classList && (node.classList.contains('w-form-done') || node
                .classList.contains('success-message')) ||
              node.querySelector && (node.querySelector('.w-form-done') || node
        ...
        if (mutation.type === 'attributes' && mutation.target.classList) {
          if (mutation.target.classList.contains('w-form-done') ||
            ...
            (mutation.target.classList.contains('w-form-done') || mutation.target
...
function headerSearch() {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (!searchPage.contains(event.target) && !searchBtn.contains(event.target)) {
      closeSearchPage();
    }
  });
...
function filterPage() {
  ...
  window.FinsweetAttributes.push([
    ...
    (listInstances) => {
      ...
      if (listInstance.hooks && listInstance.hooks.afterRender && listInstance.hooks.afterRender
        .callbacks) {
...
function divisionDropdown() {
  const divisions = document.querySelectorAll('.c-division');
  if (!divisions.length) return;

  divisions.forEach(division => {
    const toggle = division.querySelector('.c-division-toggle');
    const list = division.querySelector('.c-division-list');
    const links = division.querySelectorAll('.c-division-link');
    const toggleText = toggle?.querySelector('.t-base.static');

    if (!toggle) return;

    toggle.addEventListener('click', (e) => {
      e.stopPropagation();
      division.classList.toggle('is-open');
    });

    document.addEventListener('click', (e) => {
      if (!division.contains(e.target)) {
        division.classList.remove('is-open');
      }
    });
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && division.classList.contains('is-open')) {
        division.classList.remove('is-open');
      }
    });
    ...
    links.forEach(link => {
      link.addEventListener('click', () => {
        // Update toggle text with selected option
        if (toggleText) {
...
function init() {
  ...
  featuresTabber();
  accordions(".c-ac-item", ".c-ac-question", ".c-ac-answer", ".c-icon.ac-arrow", false);
  pdtTabber();
  pdtTabberSlider();
  filterSettings();
  accordions(".c-filter-item", ".c-filter-toggle", ".c-filter-list", ".c-icon.filter-arrow", true,
    180);
  paginationScrollTo();
  headerTabber();
  ...
  accordions(".c-faq-item", ".c-faq-question", ".c-faq-answer", ".c-icon", true, 180);
  faqPage();
  careersJobBoard();
  returnToTop();
  accordions(".c-rh-ac", ".c-rh-ac-toggle .c-btn", ".c-rh-ac-main", ".c-icon.rh", true);
  resourcesHub();
  requestLayourForm();
  searchPage();
  headerSearch();
